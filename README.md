...
- `trading_execution.market_data` owns the side-effect-free realtime data interface catalog, model input coverage matrix, and append-only realtime capture contract. Realtime acquisition may use the same canonical data providers as historical backfill, but through distinct realtime transports such as WebSocket streams or realtime HTTP snapshots.
