...
def realtime_input_coverage_matrix() -> tuple[RealtimeModelInputCoverage, ...]:
    ...
    return (
        ...
        RealtimeModelInputCoverage(
            ...
            live_input_surface="current Layer 1-3 state stack plus Layer 4 event-failure-risk conditioning",
            ...
            coverage_status="derived_context_contract_defined_no_direct_provider_route",
            validation_role=validation_role,
            boundary_note="Layer 5 alpha confidence does not need a distinct realtime provider feed; it needs fresh Layer 1-3 state plus reviewed Layer 4 event-failure-risk conditioning and label-safe future outcomes for validation.",
        ),
        RealtimeModelInputCoverage(
            contract_type="execution_realtime_input_coverage",
            model_layer="layer_06_dynamic_risk_policy",
            model_id="model_06_dynamic_risk_policy",
            model_output="dynamic_risk_policy_state",
            live_input_surface="global market, systemic event-risk, alpha-quality, and portfolio/account replay context",
            realtime_input_groups=(
                "global_market_regime_ref",
                "systemic_event_risk_context",
                "alpha_quality_context",
                "portfolio_capacity_context",
                "risk_budget_policy_context",
            ),
            primary_realtime_sources=("derived_model_context", "execution_account_state", "alpaca", "okx"),
            required_capture_fields=common + ("account_context_ref", "upstream_context_ref"),
            coverage_status="context_contract_only_broker_account_route_deferred",
            validation_role=validation_role,
            boundary_note="Layer 6 dynamic risk policy is model-internal risk-budget context, not broker permission or an execution hard limit override.",
        ),
        RealtimeModelInputCoverage(
            contract_type="execution_realtime_input_coverage",
            model_layer="layer_07_position_projection",
            model_id="model_07_position_projection",
            model_output="position_projection_vector",
            live_input_surface="current/pending position, exposure, cost, and risk-policy context plus Layer 6 policy",
            realtime_input_groups=(
                "current_position_exposure",
                "pending_order_state",
                "portfolio_risk_budget",
                "current_cost_liquidity_context",
                "upstream_dynamic_risk_policy_ref",
            ),
            ...
            coverage_status="context_contract_only_broker_account_route_deferred",
            validation_role=validation_role,
            boundary_note="Layer 7 position projection cannot be complete until broker/account state contracts exist; market-data pieces remain separate from order mutation.",
        ),
        RealtimeModelInputCoverage(
            contract_type="execution_realtime_input_coverage",
            model_layer="layer_08_underlying_action",
            model_id="model_08_underlying_action",
            model_output="underlying_action_plan",
            live_input_surface="current underlying action feasibility context plus Layer 7 projection",
            realtime_input_groups=(
            ...
            validation_role=validation_role,
            boundary_note="Layer 8 feasibility needs current market data plus execution/account restrictions; it still emits a plan, not an order.",
        ),
        RealtimeModelInputCoverage(
            contract_type="execution_realtime_input_coverage",
            model_layer="layer_09_option_expression",
            model_id="model_09_option_expression",
            model_output="option_expression_plan",
            live_input_surface="current option-chain/expression feasibility context plus Layer 8 thesis",
            realtime_input_groups=(
            ...
            validation_role=validation_role,
            boundary_note="Layer 9 realtime coverage centers on ThetaData option-chain/quote/trade context plus current underlying observations.",
        ),
        RealtimeModelInputCoverage(
            contract_type="execution_realtime_input_coverage",
            model_layer="layer_10_event_risk_governor",
            model_id="model_10_event_risk_governor",
            model_output="event_context_vector",
            live_input_surface="current target/market event context attached to the Layer 8 direct-underlying thesis",
            realtime_input_groups=(
            ...
            validation_role=validation_role,
            boundary_note="EventRiskGovernor needs news/calendar/activity capture as Layer 10 risk-governance context over the Layer 8 thesis; some routes are trigger/catalog contracts before realtime adapters exist.",
        ),
    )
