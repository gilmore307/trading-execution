...
- `contracts.py` — `execution_realtime_data_interface` catalog for OKX, Alpaca, and ThetaData realtime routes; `execution_realtime_input_coverage` coverage rows for Layers 1-9; and `realtime_capture_contract` for append-only forward/shadow validation evidence.
