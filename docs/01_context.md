...
- broker/exchange APIs once approved.
- `trading-manager` control-plane promoted decisions.
- trading-storage execution artifact layout.
...
Development is server-hosted under `/root/projects/trading-execution`.
...
`trading-execution` should not create an independent virtual environment unless a documented exception is accepted.
