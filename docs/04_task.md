...
- Added the first execution-owned realtime monitor smoke: `scripts/execution/run_realtime_monitor_smoke.py` loads the reviewed 47-symbol Layer 1/2 ETF universe and can perform bounded read-only Alpaca snapshot observations after explicit approval and execute flag.
